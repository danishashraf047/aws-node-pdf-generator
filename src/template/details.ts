...
function createMewaLogo(): React.ReactElement {
  ...
  const p = React.createElement('PATH', {
    d:
  ...
  const g3 = React.createElement(
    'G',
    { id: 'MEWA_Textil-Management_Logo-(1)', transform: 'translate(438.000000, 29.000000)' },
    g4,
  );
  const g2 = React.createElement(
    'G',
    { id: '1-DE', transform: 'translate(-438.000000, -29.000000)' },
    g3,
  );
  const g1 = React.createElement(
    'G',
...
function createArticle(article: Article, index: number, dictionary: {[p:string]: string}): React.ReactElement {
  ...
  const column1 = React.createElement(
    'VIEW',
    { key: 1, style: [styles.column, { borderRight: '0.15vw solid #001667' }] },
    [attr1, attr2, attr3, attr4],
  );
  const val1Text = React.createElement('TEXT', { style: styles.smallText }, article.color);
  const val1 = React.createElement(
    'VIEW',
    { style: [styles.cell, styles.configurationItemValue], key: 11 },
    val1Text,
  );
  ...
  const val2 = React.createElement(
    'VIEW',
    { style: [styles.cell, styles.configurationItemValue, styles.nextCell], key: 12 },
    val2Text,
  );
  const val3Text = React.createElement('TEXT', { style: styles.smallText }, article.weight);
  const val3 = React.createElement(
    'VIEW',
    { style: [styles.cell, styles.configurationItemValue, styles.nextCell], key: 13 },
    val3Text,
  );
  const val4Text = React.createElement('TEXT', { style: styles.smallText }, article.articleNo);
  const val4 = React.createElement(
    'VIEW',
    { style: [styles.cell, styles.configurationItemValue, styles.nextCell], key: 14 },
    val4Text,
  );
  const column2 = React.createElement('VIEW', { key: 2, style: styles.column }, [
    val1,
    val2,
    val3,
    val4,
  ...
  const title = React.createElement(
    'TEXT',
    { key: 1, style: [styles.configurationItemName, styles.subtitle] },
    article.name,
  );
  const content = React.createElement('VIEW', { key: 2, style: styles.configurationItemContent }, [
    column1,
    column2,
  ]);
...
function createNote(note: string, dictionary: {[p:string]: string}): React.ReactElement {
  ...
  const noteContent = React.createElement(
    'VIEW',
    { key: 2, style: styles.configurationItemContent },
    noteSpace,
  );
...
function createDetailsView(title: string, articles: Article[], note: string, dictionary: {[p: string]: string}): React.ReactElement {
  const titleText = React.createElement(
    'TEXT',
    { style: [styles.configurationName, styles.title], key: 1 },
    title,
  );
  const items = [];
  ...
  const configurationItems = React.createElement(
    'VIEW',
    { key: 2, style: styles.configurationItems },
    items,
  );
...
function createDetailsPage(props: DetailsProps): React.ReactElement {
  const { note, title, texts, articles, snapshot } = props;


  return React.createElement('VIEW', { style: styles.root, key: 'page' }, [
